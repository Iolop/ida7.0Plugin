    ...
    def _is_bad_instruction(self, ea, bad_instructions=['j', 'b'], no_clobber=[]):
        ...
        if mnem and mnem[0] in bad_instructions:
            ...
            for register in no_clobber:
                insn_var=idaapi.insn_t()
                if (insn_var.get_canon_feature() & idaapi.CF_CHG1) == idaapi.CF_CHG1:
...
class mipsropfinder_t(idaapi.plugin_t):
    ...
    def init(self):
        ...
        self.menu_context = idaapi.attach_action_to_menu("Search/", "my:actionimipsrop", idaapi.SETMENU_APP)
        return idaapi.PLUGIN_KEEP

    def term(self):
        idaapi.detach_action_from_menu("Search/", "my:actionimipsrop")
        return None
